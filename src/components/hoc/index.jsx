...
export function withCardActions(WrappedComponent) {
  return (props) => {
    ...
    const header = children.length
      ? children.find((child) => child.type.displayName === "CardHeader")
      : children.type.displayName === "CardHeader"
      ? children
      : null;
    const { children: headerChildren, ...headerProps } = header
      ? header.props
      : {};
    const content = children.length
      ? children.filter(
          (child) => !child.type || child.type.displayName !== "CardHeader"
        )
      : children.type.displayName !== "CardHeader"
      ? children
      : null;
    ...
    return (
      <WrappedComponent
        className={className + (collapse ? " card-collapsed" : "")}
        {...restProps}
      >
      ...
      <OverlayTrigger
        ...
        overlay={({
          ...
          <div
            ...
            className={`badge ${
              current < maxLength ? "badge-success" : "badge-danger"
            }`}
          >
            {current} / {maxLength}
          </div>
...
export function editableTableHOC(WrappedComponent) {
  ...
  function storeChangedData(key, row, column, value) {
    ...
    rowData ||
      (rowData =
        cache[
          cache.push({ key: row.original[key], value: { ...row.original } }) - 1
        ]);
  ...
  return (props) => {
            ...
            if (row.original.editing) {
              ...
              return (
                <Form.Control
                  ...
                  max={column.max}
                  onChange={(e) =>
                    storeChangedData(keyField, row, colKey, e.target.value)
                  }
                />
              );
            }
            ...
            return (
              <>
                <a
                  href="#save"
                  className="save-row"
                  onClick={(e) => saveRow(e, row.original[keyField])}
                >
                ...
                <a
                  ...
                  onClick={(e) => cancelRow(e, row.original[keyField])}
                >
          ...
          return (
            <>
              <a
                href="#edit"
                className="edit-row"
                onClick={(e) => editRow(e, row.original[keyField])}
              >
              ...
              <a
                ...
                onClick={(e) => removeRow(e, row.original[keyField])}
              >
    ...
    function isValidData(rowData) {
      for (let key in rowData.value) {
        ...
        if (
          col &&
          ((col.min !== null && col.min !== undefined && col.min > val) ||
            (col.max !== null && col.max !== undefined && col.max < val))
        )
          return false;
      }
      return true;
    ...
    return <WrappedComponent columns={getExpandedColumns(columns)} {...rest} />;
  };
}
