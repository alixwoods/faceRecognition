...
export function WizardPager ( props ) {
    ...
    let content = (
        <ul className="pager">
            <li className={ `next ${ current === 0 ? 'disabled' : '' }` }>
                <Button href="#prev" variant="default" onClick={ onPrev }><i className="fas fa-angle-right"></i> قبلی</Button>
            </li>
            { current < total - 1 ?
                <li className="previous">
                    <Button href="#next"  variant="default" onClick={ onNext }>بعدی <i className="fas fa-angle-left"></i></Button>
                </li>
                : <li className="finish float-right">
                    <Button href="#finish" variant="default" onClick={ onFinish }>ثبت</Button>
                </li>
            }
        </ul>
    )
...
// var allowance=false
    useEffect( () => {
        setValidated( false );
        props.parentCallBack(curPage)
    }, [ curPage ] )

    useEffect( () => {
        if(props.myFormsVal==true){
            setAllowance(true)
        }else if(props.myFormsVal==false){
            setAllowance(false)
